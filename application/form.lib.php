...
class Form {
    ...
    private static function handleUploadFile($field){
        ...
        foreach( $files as $file ){
            ...
            if( $ok ) {
                ...
                if( !empty($link) ){
                    self::setValue( $field['cid'] . '.name' . $prefix, $fileName );
    ...
    private static function getUploadFileArray($field){
        $isMultiple = is_array($_FILES[ $field['cid']['tmp_name'] ]);
        if( !isMultiple ) 
            return $_FILES[ $field['cid'] ];
        $names = array();
        foreach( $_FILES[ $field['cid'] ] as $key => $values ){
